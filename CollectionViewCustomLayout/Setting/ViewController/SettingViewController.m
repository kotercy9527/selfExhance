...
- (void)viewDidLoad {
    ...
    self.titleArray = @[@"ScrollInScroll",@"pathAnimation",@"customLayout",@"supplementaryCollectionView",@"textAnimateView",@"算法",@"自定义CollectionViewlayout",@"FrameTest"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([title isEqualToString:@"ScrollInScroll"]) {
        ...
        vc.hidesBottomBarWhenPushed = YES;
        [self.navigationController pushViewController:vc animated:YES];
    } else if ([title isEqualToString:@"自定义CollectionViewlayout"]) {
        CustomCollectionViewController *vc = [[CustomCollectionViewController alloc] init];
        vc.hidesBottomBarWhenPushed = YES;
        [self.navigationController pushViewController:vc animated:YES];
    } else if ([title isEqualToString:@"FrameTest"]) {
        FrameTestViewController *vc = [[FrameTestViewController alloc] init];
        vc.hidesBottomBarWhenPushed = YES;
        [self.navigationController pushViewController:vc animated:YES];
    }
}
